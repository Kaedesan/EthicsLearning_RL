...
# The original hsarsa.py has been modified for learning an human policy
# generated by the DriveMixSplit class.
# Same full ethitical state for all the type of policies (neg, pos, mix)
...
parser.add_argument('--ambulance', action='store_true',
...
if args.n_ethical == True:
    dr = DrivingMixSplit(ishuman_n=True)
    policy_name = 'human_n_mix'
elif args.p_ethical == True:
    dr = DrivingMixSplit(ishuman_p=True)
    policy_name = 'human_p_mix'
else:
    if args.ambulance == True:
        dr = DrivingMixSplit(ishuman_m=True, ambulance_m = True)
    else:
        dr = DrivingMixSplit(ishuman_m=True)
    policy_name = 'human_m_mix_split'
...
for cnt in range(args.num_episodes):
    ...
    while True:
        ...
        if args.n_ethical == True:
            ...
            if args.ambulance == True:
                # With the number of passengers in the state
...
if args.first_ep_rec > 0:
    ...
    beg_ambulances_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_ambulance_hits_begining.csv'.format(args.id, args.first_ep_rec, args.temp, args.gamma, policy_name), index=False)
...
if args.last_ep_rec > 0:
    ...
    end_ambulances_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_ambulance_hits_end.csv'.format(args.id, args.last_ep_rec, args.temp, args.gamma, policy_name), index=False)
...
dfa.to_csv('./record/{}_{:.2f}_{:.2f}_{}_ambulance_hits.csv'.format(args.id, args.cn, args.taun, policy_name), index=False)
