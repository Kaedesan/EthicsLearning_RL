...
# previous representation to take care of both. But this time the same
# state is used for all the policies.

class DrivingMix2(object):
    def __init__(self, num_lanes=5, p_car=0.1, p_cat=0.045, p_elder=0.045, sim_len=300, ishuman_n=False, ishuman_p=False, ishuman_m=False):
    ...
    def state_generator(self):
        ...
        if self.lane < self.num_lanes-1: # there is a lane on the right
            ...
            self.state += (-2, -2, -2) # if already on the last
    ...
    def step(self, action):
        ...
        if self.ishuman_n: # building the human policy for "Driving and avoiding" = negative reward if crossing the object
            if self.num_saved_elder > 0:
                reward = -(20/self.num_saved_elder) * cat_hit + -1 * car_hit + 0.5 * (action == 0)
                #reward = -(15/self.num_saved_elder) * cat_hit + -1 * car_hit + 0.5 * (action == 0)
            else:
                #reward = -15 * cat_hit + -1 * car_hit + 0.5 * (action == 0)
            ...
            reward = 30 * elder_saved + -1 * car_hit + 0.5 * (action == 0)

        elif self.ishuman_m:
            if self.num_saved_elder > 0: # mean that the agent carry an elderly, it is more important
                #reward = -(10 + self.num_saved_elder) * car_hit + 0.5 * (action == 0) + 20 * elder_saved - (10/self.num_saved_elder) * cat_hit
                reward = -10 * car_hit + 0.5 * (action == 0) + 30 * elder_saved - (15/self.num_saved_elder) * cat_hit
            else:
                reward = -1 * car_hit + 0.5 * (action == 0) + 30 * elder_saved -15 * cat_hit
        else:
