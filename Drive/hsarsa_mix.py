...
# The original hsarsa.py has been modified for learning from a mixed policy
# Different ethitical and general states for all the type of ploicies (neg, pos, mix)
...
if args.n_ethical == True:
    dr = DrivingMix(ishuman_n=True)
    policy_name = 'human_n_mix'
elif args.p_ethical == True:
    dr = DrivingMix(ishuman_p=True)
    policy_name = 'human_p_mix'
else:
    dr = DrivingMix(ishuman_m=True)
...
for cnt in range(args.num_episodes):
    ...
    while True:
        ...
        if args.n_ethical == True:
            #If full environnement state space is used
            #ethical_state = (state[2], state[5], state[8])
            #Classical ethic state
    ...
    # Morality price (refer to the report)
    m_price = 0
    for pair in Q:
...
if args.first_ep_rec > 0:
    ...
    beg_col_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_collisions_begining.csv'.format(args.id, args.first_ep_rec, args.temp, args.gamma, policy_name), index=False)
    beg_cats_ev = pd.DataFrame(np.array(beg_record_cat_hits))
    beg_cats_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_cat_hits_begining.csv'.format(args.id, args.first_ep_rec, args.temp, args.gamma, policy_name), index=False)
    beg_elders_ev = pd.DataFrame(np.array(beg_record_elders_saved))
    beg_elders_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_elders_saved_begining.csv'.format(args.id, args.first_ep_rec, args.temp, args.gamma, policy_name), index=False)
...
if args.last_ep_rec > 0:
    ...
    end_col_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_collisions_end.csv'.format(args.id, args.last_ep_rec, args.temp, args.gamma, policy_name), index=False)
    end_cats_ev = pd.DataFrame(np.array(end_record_cat_hits))
    end_cats_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_cat_hits_end.csv'.format(args.id, args.last_ep_rec, args.temp, args.gamma, policy_name), index=False)
    end_elders_ev = pd.DataFrame(np.array(end_record_elders_saved))
    end_elders_ev.to_csv('./record_timesteps/{}_{}_{:.2f}_{:.2f}_{}_elders_saved_end.csv'.format(args.id, args.last_ep_rec, args.temp, args.gamma, policy_name), index=False)
...
dfmp.to_csv('./record/{}_{:.2f}_{:.2f}_{:.2f}_{:.2f}_{}_morality_prices.csv'.format(args.id, args.cn, args.taun, args.cp, args.taup, policy_name), index=False)
